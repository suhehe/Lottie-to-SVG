let globalAssets = {};
    ...
    let currentRawSvgOutput = '';
    let currentCompressedSvgOutput = '';
    let currentSvgVariant = 'raw';
    let preferCompressedSvg = true;
    ...
    function getActiveSvgOutput() {
        const resolvedVariant = preferCompressedSvg && currentCompressedSvgOutput ? 'compressed' : 'raw';
        currentSvgVariant = resolvedVariant;
        return resolvedVariant === 'compressed' && currentCompressedSvgOutput
            ? currentCompressedSvgOutput
            : currentRawSvgOutput;
    }

    function setSvgCompression(enabled) {
        preferCompressedSvg = Boolean(enabled);
        currentSvgVariant = preferCompressedSvg && currentCompressedSvgOutput ? 'compressed' : 'raw';
        renderActiveSvgOutput();
        updateSvgVariantControls();
    }

    function toggleSvgCompression() {
        setSvgCompression(!preferCompressedSvg);
    }
    ...
    function updateSvgVariantControls() {
        ...
        const hasCompressed = !!currentCompressedSvgOutput;
        if (toggle) {
            toggle.disabled = !currentRawSvgOutput;
            toggle.setAttribute('aria-checked', String(preferCompressedSvg));
            toggle.title = hasCompressed
                ? (preferCompressedSvg ? '当前显示压缩版 SVG' : '当前显示原始版 SVG')
  ...
  ${bodyHTML}
        ...
        currentRawSvgOutput = finalSVG;
        currentCompressedSvgOutput = compressedSVG && getByteSize(compressedSVG) < getByteSize(finalSVG) ? compressedSVG : '';
        currentSvgVariant = preferCompressedSvg && currentCompressedSvgOutput ? 'compressed' : 'raw';
    ...
    function downloadSVG() {
        ...
        a.href = url;
        a.download = currentSvgVariant === 'compressed' ? 'compiled-animation.min.svg' : 'compiled-animation.svg';
        a.click();
    }
